...
/* Created on:     10/20/2013 3:04:38 PM                        */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('POSLOVNI_OBJEKAT') and o.name = 'FK_POSLOVNI_PRED_POS4_PREDUZEC')
alter table POSLOVNI_OBJEKAT
   drop constraint FK_POSLOVNI_PRED_POS4_PREDUZEC
go
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('POSLOVNI_PARTNER') and o.name = 'FK_POSLOVNI_PRED_POS3_PREDUZEC')
alter table POSLOVNI_PARTNER
   drop constraint FK_POSLOVNI_PRED_POS3_PREDUZEC
go
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('PROMETNI_DOKUMENT') and o.name = 'FK_PROMETNI_POS_OBJ_P_POSLOVNI_2')
alter table PROMETNI_DOKUMENT
   drop constraint FK_PROMETNI_POS_OBJ_P_POSLOVNI_2
go
...
if exists (select 1
           ...
           where  id    = object_id('ARTIKAL')
            and   name  = 'ARTIKAL_PREDUZECA_FK'
            and   indid > 0
            and   indid < 255)
   drop index ARTIKAL.ARTIKAL_PREDUZECA_FK
go
...
if exists (select 1
           ...
           where  id    = object_id('POSEDOVANJE')
            and   name  = 'POS_OBJ_JE_SADRZAN_FK'
            and   indid > 0
            and   indid < 255)
   drop index POSEDOVANJE.POS_OBJ_JE_SADRZAN_FK
go

if exists (select 1
            from  sysindexes
           where  id    = object_id('POSEDOVANJE')
            and   name  = 'POS_OBJ_SADRZI_FK'
            and   indid > 0
            and   indid < 255)
   drop index POSEDOVANJE.POS_OBJ_SADRZI_FK
go
...
if exists (select 1
           ...
           where  id    = object_id('PREDUZECE')
            and   name  = 'PREDUZECE_U_MESTU_FK'
            and   indid > 0
            and   indid < 255)
   drop index PREDUZECE.PREDUZECE_U_MESTU_FK
go
...
create table ANALITIKA_MAGACINSKE_KARTICE (
   ...
   constraint PK_ANALITIKA_MAGACINSKE_KARTIC primary key nonclustered (POS_PIB, SIFRA_OBJEKTA, PIB, SIFRA_ARTIKLA, POS_PIB2, MAG_GODINA, RBR_ANALITIKE_)
)
go
...
/*==============================================================*/
/* Index: ARTIKAL_PREDUZECA_FK                                  */
/*==============================================================*/
create index ARTIKAL_PREDUZECA_FK on ARTIKAL (
PIB ASC
)
go
...
create table CLAN_POPISNE_KOMISIJE (
   ...
   constraint PK_CLAN_POPISNE_KOMISIJE primary key (POS_PIB, GODINA, PIB, SIFRA_OBJEKTA, BROJ_POPISA, ZAP_PIB, SIFRARDNK)
)
go
...
create table MAGACINSAK_KARTICA (
   ...
   constraint PK_MAGACINSAK_KARTICA primary key (POS_PIB, SIFRA_OBJEKTA, PIB, SIFRA_ARTIKLA, POS_PIB2, GODINA)
)
go
...
create table POPISNI_DOKUMENT (
   ...
   STATUS               char(1)              not null default 'f'
      constraint CKC_STATUS_POPISNI_ check (STATUS in ('f','p','s')),
   constraint PK_POPISNI_DOKUMENT primary key nonclustered (POS_PIB, GODINA, PIB, SIFRA_OBJEKTA, BROJ_POPISA)
)
go
...
create table POSEDOVANJE (
   PIB                  char(12)             not null,
   POS_SIFRA_OBJEKTA    char(12)             not null,
   POS_PIB              char(12)             not null,
   SIFRA_OBJEKTA        char(12)             not null,
   constraint PK_POSEDOVANJE primary key (PIB, POS_SIFRA_OBJEKTA, POS_PIB, SIFRA_OBJEKTA)
)
go

/*==============================================================*/
/* Index: POS_OBJ_SADRZI_FK                                     */
/*==============================================================*/
create index POS_OBJ_SADRZI_FK on POSEDOVANJE (
PIB ASC,
POS_SIFRA_OBJEKTA ASC
)
go

/*==============================================================*/
/* Index: POS_OBJ_JE_SADRZAN_FK                                 */
/*==============================================================*/
create index POS_OBJ_JE_SADRZAN_FK on POSEDOVANJE (
POS_PIB ASC,
SIFRA_OBJEKTA ASC
)
go
...
/*==============================================================*/
/* Index: PREDUZECE_U_MESTU_FK                                  */
/*==============================================================*/
create index PREDUZECE_U_MESTU_FK on PREDUZECE (
SIFRA_DRZAVE ASC,
SIFRA_MESTA ASC
)
go
...
create table PROMETNI_DOKUMENT (
   ...
   PIB                  char(12)             null,
   PIBPP                char(12)             null,
   POS_PIB2             char(12)             null,
   POS_SIFRA_OBJEKTA    char(12)             null,
   ...
   constraint PK_PROMETNI_DOKUMENT primary key nonclustered (POS_PIB3, GODINA, BROJ_PROMETNOG_DOKUMENTA)
)
go
...
create table STAVKA_PROMETNOG_DOKUMENTA (
   ...
   constraint PK_STAVKA_PROMETNOG_DOKUMENTA primary key (POS_PIB3, GODINA, BROJ_PROMETNOG_DOKUMENTA)
)
go
...
create table STAVKE_POPISA (
   ...
   constraint PK_STAVKE_POPISA primary key (POS_PIB, GODINA, POP_PIB, SIFRA_OBJEKTA, BROJ_POPISA, PIB, SIFRA_ARTIKLA)
)
go
...
alter table ANALITIKA_MAGACINSKE_KARTICE
   add constraint FK_ANALITIK_MAG_KAR_A_MAGACINS foreign key (POS_PIB, SIFRA_OBJEKTA, PIB, SIFRA_ARTIKLA, POS_PIB2, MAG_GODINA)
      references MAGACINSAK_KARTICA (POS_PIB, SIFRA_OBJEKTA, PIB, SIFRA_ARTIKLA, POS_PIB2, GODINA)
go

alter table ANALITIKA_MAGACINSKE_KARTICE
   add constraint FK_ANALITIK_STAV_PRO__STAVKA_P foreign key (POS_PIB3, GODINA, BROJ_PROMETNOG_DOKUMENTA)
      references STAVKA_PROMETNOG_DOKUMENTA (POS_PIB3, GODINA, BROJ_PROMETNOG_DOKUMENTA)
go
...
alter table CLAN_POPISNE_KOMISIJE
   add constraint FK_CLAN_POP_POP_DOK_C_POPISNI_ foreign key (POS_PIB, GODINA, PIB, SIFRA_OBJEKTA, BROJ_POPISA)
      references POPISNI_DOKUMENT (POS_PIB, GODINA, PIB, SIFRA_OBJEKTA, BROJ_POPISA)
go

alter table CLAN_POPISNE_KOMISIJE
   add constraint FK_CLAN_POP_ZAP_CLN_P_ZAPOSLEN foreign key (ZAP_PIB, SIFRARDNK)
...
alter table MAGACINSAK_KARTICA
   add constraint FK_MAGACINS_POS_GOD_M_POSLOVNA foreign key (POS_PIB2, GODINA)
      references POSLOVNA_GODINA (PIB, GODINA)
go

alter table MAGACINSAK_KARTICA
   add constraint FK_MAGACINS_POS_OBJ_M_POSLOVNI foreign key (POS_PIB, SIFRA_OBJEKTA)
...
alter table POPISNI_DOKUMENT
   add constraint FK_POPISNI__POS_GOD_P_POSLOVNA foreign key (POS_PIB, GODINA)
      references POSLOVNA_GODINA (PIB, GODINA)
go
...
alter table POSEDOVANJE
   add constraint FK_POSEDOVA_POS_OBJ_J_POSLOVNI foreign key (POS_PIB, SIFRA_OBJEKTA)
      references POSLOVNI_OBJEKAT (PIB, SIFRA_OBJEKTA)
go

alter table POSEDOVANJE
   add constraint FK_POSEDOVA_POS_OBJ_S_POSLOVNI foreign key (PIB, POS_SIFRA_OBJEKTA)
      references POSLOVNI_OBJEKAT (PIB, SIFRA_OBJEKTA)
go
...
alter table POSLOVNI_OBJEKAT
   add constraint FK_POSLOVNI_PRED_POS4_PREDUZEC foreign key (PIB)
      references PREDUZECE (PIB)
go
...
alter table POSLOVNI_PARTNER
   add constraint FK_POSLOVNI_PRED_POS3_PREDUZEC foreign key (PIB)
      references PREDUZECE (PIB)
go
...
alter table PROMETNI_DOKUMENT
   add constraint FK_PROMETNI_POS_GOD_P_POSLOVNA foreign key (POS_PIB3, GODINA)
      references POSLOVNA_GODINA (PIB, GODINA)
go

alter table PROMETNI_DOKUMENT
   add constraint FK_PROMETNI_POS_OBJ_P_POSLOVNI_2 foreign key (POS_PIB, SIFRA_OBJEKTA)
      references POSLOVNI_OBJEKAT (PIB, SIFRA_OBJEKTA)
go

alter table PROMETNI_DOKUMENT
   add constraint FK_PROMETNI_POS_OBJ_P_POSLOVNI foreign key (POS_PIB2, POS_SIFRA_OBJEKTA)
      references POSLOVNI_OBJEKAT (PIB, SIFRA_OBJEKTA)
go
...
alter table STAVKA_PROMETNOG_DOKUMENTA
   add constraint FK_STAVKA_P_STAV_PRO__PROMETNI foreign key (POS_PIB3, GODINA, BROJ_PROMETNOG_DOKUMENTA)
      references PROMETNI_DOKUMENT (POS_PIB3, GODINA, BROJ_PROMETNOG_DOKUMENTA)
go

alter table STAVKE_POPISA
   add constraint FK_STAVKE_P_POP_DOK_S_POPISNI_ foreign key (POS_PIB, GODINA, POP_PIB, SIFRA_OBJEKTA, BROJ_POPISA)
      references POPISNI_DOKUMENT (POS_PIB, GODINA, PIB, SIFRA_OBJEKTA, BROJ_POPISA)
go
